...
Alt.OngoingAdvice.Models.OngoingAdviceModel = function(initialData) {
    ...
    // Task management
    self.adviceTasks = ko.observable([]);
    self.taskManagementStatus = ko.observable(null);
    self.hasAdviceTasks = ko.pureComputed(function() {
        var tasks = self.adviceTasks();
...
/**
 * Update advice tasks information
 */
Alt.OngoingAdvice.Models.OngoingAdviceModel.prototype.updateAdviceTasks = function(tasks) {
    var self = this;
    
    self.adviceTasks(tasks || []);
    
    if (tasks && tasks.length > 0) {
        var taskSummary = tasks.length + ' advice task' + (tasks.length > 1 ? 's' : '') + ' found';
        self.taskManagementStatus(taskSummary);
    } else {
        self.taskManagementStatus('No advice tasks found');
    }
};
